 ...
 *
 * `backend/src/lib/admin-bridge-token.ts` is the source of truth — the
 ...
 * two files must agree byte for byte on the wire format. Change the BACKEND
 * copy first and mirror it here; its own spec
 * (`backend/src/lib/__tests__/admin-bridge-token.unit.spec.ts`) pins the format.
 *
 * ## The format
 *
